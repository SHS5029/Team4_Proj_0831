...
관리자 앱은 read-only `운영 분석`, `사용자 피드백`, `관리자 로그`, `운영 에이전트 계획`
탭으로 구성됩니다. 운영 분석에는 사용자 수·진행 게임 수·진영별 승률 KPI와 에이전트
페르소나별 승률 그래프를 함께 표시하고, 피드백과 감사 로그는 별도 탭에서 조회합니다.
운영 에이전트 계획 화면은 승인된 피드백·공개 게임 요약·운영 문서만 검색해 근거와
함께 요약하는 RAG 방향을 설명하며, 관리자 데이터 변경이나 자동 조치는 수행하지
않습니다. 실제 관리자 접근은 `ADMIN_USER_IDS` allowlist와 Backend 응답으로 판정하고,
가상 API 키는 데모 연결 확인에만 사용합니다.
