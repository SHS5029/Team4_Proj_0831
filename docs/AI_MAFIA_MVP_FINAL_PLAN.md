...
이 문서는 세 기획 문서를 현재 저장소 디렉터리 구조 기준으로 검증·통합한
최종 MVP 플랜이다. 세 원본 문서와 이 문서의 내용이 충돌하면 이 문서를
우선한다.
...
첫 버전은 **마피아, 경찰, 의사, 시민** 네 역할만 사용한다. 역할 규칙, 인원
구성, 페르소나를 데이터로 분리해 이후 새 직업, 추리극 시나리오(사건 패턴,
단서, Red Herring, AI GM 해설), 다인 플레이로 확장할 수 있게 한다.
...
- **5~9명** 게임 생성: 인간 1명, 나머지는 AI
- 기본 역할 자동 배정과 비공개 역할 안내
- 밤 행동, 낮 토론, 투표, 처형, 승패 판정
- AI 플레이어 페르소나와 중재 에이전트(진행 안내 전용)
...
└─────────┬──────────┘       └─────────┬──────────┘
  ...
  │ 원본/이벤트  │ │ lock/cache   │ │ OpenAI/Gemini│
  └──────────────┘ └──────────────┘ └──────┬───────┘
                                           │ 허용 목록만
                                           ▼
                                ┌──────────────────────┐
...
| `backend/app/agent/` | Agent Manager: 플레이어·중재 에이전트 실행 조율 |
...
역할은 인간에게도 완전 무작위 배정한다. 운영 재현을 위한 random seed는
서버에 기록하되 일반 응답에는 노출하지 않는다. 게임 생성자는 게임 소유자라는
메타정보만 가지며 별도의 플레이어 권한(호스트 등급)은 없다.

### 4.3 게임 진행

- 상단: 라운드, 현재 단계, 생존자 수, 저장 상태
- 중앙: 중재자의 안내와 공개 대화 타임라인
- 하단: 현재 역할과 단계에 허용된 인간 행동 (버튼형 턴제)
- 사이드 패널: 생존 참가자, 공개된 사망자와 메모
- `저장하고 나가기`와 마지막 자동 저장 시각

인간이 탈락하면 공개 정보만 보는 관전 상태로 전환한다. `AI 진행 보기`와
`결과까지 빠르게 진행`을 제공해 대기 시간을 줄인다.
...
인간의 생존 여부와 투표 적중률을 표시한다. `한 판 더`, `홈으로`,
`피드백 남기기`를 제공한다. 게임 종료 전에는 전체 역할을 절대 반환하지 않는다.
...
| 역할 | 진영 | 밤 행동 | 승리 조건 |
|---|---|---|---|
| 마피아 | 마피아 | 생존자 1명 제거 선택 | 마피아 수가 비마피아 생존자 수 이상 |
| 경찰 | 시민 | 생존자 1명의 마피아 진영 여부 조사 | 모든 마피아 제거 |
| 의사 | 시민 | 생존자 1명 보호 | 모든 마피아 제거 |
| 시민 | 시민 | 없음 | 모든 마피아 제거 |

- 경찰 조사 결과는 `마피아 진영` 또는 `비마피아 진영`으로만 전달하고 정확한
  역할명은 알려주지 않는다.
- **의사는 자기 보호와 연속 동일 대상 보호를 허용한다** (구현 단순화).
- 마피아가 2명이면 서로를 알고 협력하며, 각자의 선택을 다수결로 정하고
  동률이면 서버의 결정적 규칙으로 한 명을 선택한다. 주범·공범 구분은 없다.
- 규칙은 `ruleset_version`에 묶어 저장본 복구 중 바뀌지 않게 한다.
...
CREATED
  ...
  → DAY_VOTE
  → VOTE_RESOLUTION
       ├─ 동률 → DAY_REVOTE(동점자 대상 1회) → VOTE_RESOLUTION
       ├─ 승패 미확정 → NIGHT_ACTION
       └─ 승패 확정 → FINISHED
```
...
밤에는 마피아 대상, 의사 보호, 경찰 조사를 받은 뒤 공격과 보호가 같으면
사망을 취소한다. 경찰 결과는 해당 경찰의 private event로만 저장한다.
중재자는 사망 여부만 공개하며 보호 대상과 조사 결과를 누설하지 않는다.

낮에는 밤 결과 공개, 제한된 공개 발언, 생존자 투표, 처형과 승패 판정을
진행한다.

### 6.4 투표 규칙

- 투표는 비공개로 진행하고 결과 집계만 공개한다.
...
- 매 밤 해소와 투표 해소 뒤 시민 또는 마피아 승리 조건을 확인한다.
...
**중재 에이전트**는 규칙 엔진이 만든 공개 결과를 자연어로 전달하고 발언
순서를 안내한다. 상태를 직접 변경하거나 숨겨진 역할을 조회하지 않는다.
MVP의 중재자는 진행 안내 전용이며 사건 해설(추리극 GM)은 후속 확장이다.
...
문자를 검증한다. MVP는 차분한 분석가, 성급한 리더, 조용한 관찰자, 친화적
중재자, 능숙한 블러퍼 같은 서버 등록 프리셋을 무작위 배정한다(최소 3종 이상).
...
```text
공통 정보: 게임 룰, 현재 라운드·단계·규칙 버전, 생존자 목록,
          공개 발언·투표·사망 결과
개인 정보: 자신의 role과 생존 상태, persona, 자기 private event,
          자기 메모리, 현재 허용 action 목록
```

다른 참가자의 role, 다른 경찰의 조사 결과, 보호 대상, random seed, 전체 게임
...
- Backend가 `game_id`, `agent_id`, phase 기준으로 capability를 발급하고 허용
  목록 밖의 호출을 거부한다.
- `agent_id`는 MCP 연결 생성 시 서버가 고정한다. 에이전트가 Tool 입력으로
  `actor_id`나 자신의 `agent_id`를 보내지 않는다.
- 다른 `agent_id`를 지정할 수 있는 API를 제공하지 않는다. `me`는 연결된
  세션 기준으로 해석한다.
- 게임 재시작 시 새로운 `session_id`를 발급하고, 다른 게임의 메모리와
  이벤트가 현재 게임에 포함되지 않게 한다.
...
| URI | 내용 |
|---|---|
| `mafia://rules/basic-v1` | 현재 규칙, 역할, 페이즈와 승리 조건 |
| `mafia://games/{game_id}/public-state` | 페이즈, 라운드, 생존자·사망자 |
| `mafia://games/{game_id}/public-timeline` | 공개 사건과 발언 기록 |
| `mafia://games/{game_id}/agents/me` | 자기 정보(역할, 생존, 표시명) |
| `mafia://games/{game_id}/agents/me/private-state` | 자기 private event·메모리 |
...
| `game.investigate` | 경찰 조사 제안 | 경찰 역할, 밤 페이즈, 유효 대상, 중복 |
| `game.protect` | 의사 보호 제안 | 의사 역할, 밤 페이즈, 유효 대상 |
| `game.end_turn` | 명시적 턴 종료 | 현재 턴 소유 여부 |

- Tool은 상태를 직접 변경하지 않고 Game Service가 검증할 **행동 제안**을
  반환한다. 실제 event append와 상태 전이는 Backend 트랜잭션에서만 수행한다.
- `game.kill`이 호출되어도 실제 사망 여부는 엔진이 보호·중복 규칙을 계산해
  결정한다. 조사 결과는 호출 경찰에게만 비공개로 반환한다.
...
모든 Resource 조회와 Tool 호출은 `request_id`, `game_id`, `agent_id`,
`session_id`, 작업 유형·이름, 허용 여부, 결과 event id, 시각으로 기록한다.
감사 로그는 AI 에이전트가 조회할 수 없다.
...
| `audit_logs` | id, trace_id, actor_type, actor_id_hash, action, target_type, target_id, metadata, created_at | 관리자·보안·MCP 감사 |

이벤트 공개 범위(visibility)는 네 가지로 분리한다.

- `PUBLIC`: 모든 참가자 (공개 발언, 투표 집계, 사망 발표, 페이즈 전환, 종료)
- `PRIVATE`: 지정 에이전트 (역할 배정, 조사 결과, 행동 접수 결과)
- `FACTION`: 마피아 합의용 진영 공유 정보
- `SYSTEM`: 엔진 전용 (전체 역할표, 야간 행동 원본, 판정 과정, 내부 오류)

중요 사용자 행동과 phase 종료 직후 자동 저장한다. `저장하고 나가기`는 agent
run이 끝난 안전 지점에서 `PAUSED`로 전환한다. snapshot checksum과 마지막
event sequence가 다르면 이벤트 재생으로 복구한다. 검증 후 채택된 발언과
행동만 원본으로 저장한다.
...
- 게임 API에는 검증 가능한 사용자 세션 또는 짧은 수명의 Backend access
  token을 추가하고 모든 조회에서 소유권을 검사한다.
...
- 사람 플레이어가 접속을 종료하면 게임을 `PAUSED`로 저장하고 재접속 시 기존
  역할과 좌석으로 이어간다. MVP에서 사람 좌석을 AI로 자동 교체하지 않는다.
...
사용자 명령 또는 phase 시작
  ...
  → 규칙 엔진이 허용 행동과 대상 계산
  → 에이전트별 최소 컨텍스트 구성 (격리 검사 포함)
  → MCP 허용 범위(capability) 설정
  → LLM 구조화 결정 요청
  → schema·규칙·안전 정책 검증
...
좌석 순서대로 진행한다. 중재 에이전트는 공개 event가 확정된 뒤 호출한다.

## 14. 구현 단계

### 0단계: 계약 고정 (완료 사항 반영)

- 인원 5~9명, 역할표, 의사 규칙(자기·연속 보호 허용), 투표 동률 규칙
...
필수 자동 테스트: 역할 배정 불변식, 단계별 행동 거부, 의사·경찰·마피아 밤
처리, 투표 동률·재투표, 승패 경계, snapshot/event 재생, optimistic lock,
idempotency, Redis lock, 게임 소유권, 관리자 권한, 에이전트 정보 누설
(카나리·비간섭성), LLM/MCP 실패 fallback, 입력 escape와 민감 로그 제거.

통합 테스트는 생성 → 역할 확인 → 밤 → 낮 → 투표 → 저장 → 재개 → 결과
흐름과 관리자 KPI 집계를 synthetic 데이터와 fake LLM/MCP로 검증한다. 유료
LLM을 기본 회귀 테스트에서 호출하지 않는다.

MVP 완료 조건:

- 로그인한 사용자 한 명이 5~9명 게임을 생성·완주한다.
...
1. 사용자 게임 API 인증 방식 (세션 vs 짧은 수명 access token)
2. 게임당 token·비용 상한과 목표 플레이 시간
3. 저장본 보존 기간과 사용자 삭제 기능 제공 시점

초기 권장값은 **발언 버튼형 턴제, 10~15분 목표, 저장본 30일 보존**이다.
턴제는 한 명 플레이에서 읽는 속도를 보장하고 예상하지 못한 LLM 연속 호출과
비용을 제어하기 쉽다.
