...
모델의 자발적 PASS와 장애로 인한 PASS가 구분되어야 한다. 장애는 펼치지 않아도
확인 가능하고 상세 최근 기록은 기존 expander에서 UTC 시각·단계·근거를 표시한다.
...
- `뒤로가기`는 앱 내부에서 직전에 방문한 화면으로 이동하고 방문 기록이 없으면 홈으로
  이동한다. `홈`은 방문 기록을 비우고 홈으로 이동한다. 두 이동은 command를 제출하거나
  진행 중인 게임·작성 중인 입력·결과 불명 요청을 삭제하지 않으며 홈 목록 cache만
  무효화해 최신 상태를 다시 조회한다.
