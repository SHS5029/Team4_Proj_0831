...
현재 관리자 센터는 중복을 줄인 다음 세 화면으로 구성됩니다.

- **운영 분석**: 전체 사용자·누적 게임·완료율·평균 경기 라운드·시민/마피아 승률 KPI, 진영별 도넛, 에이전트 페르소나별 승률 그래프와 일별 막대 추이(최근 30일 총합·일평균·최다 생성일 포함, 요약 카드 상승 배지와 날짜별 노란색 강조, 막대 위 수치 라벨 흰색 처리)
- **사용자 피드백**: 평균 피드백 점수, 종류/평점 필터와 사용자 의견
- **관리자 로그**: 조회 유형별 감사 기록과 요청·대상 식별자
- 세 화면은 같은 관리자용 짙은 퍼플 색상 체계를 사용하며, 차트·KPI·입력창의 글자
...
실제 모드의 페르소나별 승률, 피드백 목록, 감사 로그는 API 명세 7.4~7.8과 연결됩니다.
...
피드백과 감사 로그는 20건씩 조회하며 `← 이전 · 페이지·건수 · 다음 →` 한 줄 컨트롤로 이동합니다. 필터 변경 시 첫 페이지로 돌아갑니다.
감사 이벤트를 조회한 기록은 다음 새로고침 때 보입니다.
관리자 로그의 조회 유형은 현재 화면에서 실제로 연동하는 운영 지표·페르소나 승률·피드백 목록·감사 로그만 제공합니다.
