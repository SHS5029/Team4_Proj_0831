...
class FastMcpGameContextClient:
    ...
    async def _rpc(self, method: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if body.get("jsonrpc") != "2.0" or type(body.get("id")) is not int or body["id"] != self._request_id or "error" in body or not isinstance(body.get("result"), dict):
    ...
    def _validate_context(self, payload: dict[str, Any], scope: str) -> None:
        ...
        try:
            ...
            if payload["phase"] not in phases or (self._phase is not None and payload["phase"] != self._phase):
                raise ValueError
            ...
            raise RuntimeError("FastMCP context contract is invalid") from error
