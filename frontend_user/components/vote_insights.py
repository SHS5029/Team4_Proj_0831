"""공개 AI 발언 분석을 투표 상태와 분리된 읽기 전용 보조 화면으로 표시한다."""
...
def _load(*, client: Any, game_id: str, window_id: str, scope: str) -> dict[str, Any] | None:
    """동일 창의 완료 응답은 재사용하고 처리 중인 응답만 기존 rerun에서 갱신한다."""

    identity = (str(getattr(client, "user_id", "")), game_id, window_id, scope)
    cached = st.session_state.get(CACHE_KEY, {})
    if cached.get("identity") == identity and cached.get("status") not in {"PENDING", "PARTIAL"}:
...
def render(*, client: Any, game_id: str, snapshot: dict[str, Any]) -> None:
    ...
    if game.get("phase") not in VOTE_PHASES:
        st.session_state.pop(CACHE_KEY, None)
        return
    with st.expander("AI 발언 돌아보기", expanded=False):
        st.caption("공개 AI 발언을 분석한 참고 정보이며 사실 판정이 아닙니다. AI 발언 기준입니다.")
        ...
        scope = st.radio("분석 범위", list(SCOPES), format_func=SCOPES.get,
                         key=f"vote_insights.scope.{game_id}.{window['window_id']}", horizontal=True)
        ...
        st.text(f"{SCOPES[scope]} · AI 발언 {total}개 · "
