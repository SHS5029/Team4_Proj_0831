...
from frontend_user.auth.authorization import (
    APPLICATION_ACCESS_SESSION_KEY,
    should_process_external_user,
)
from frontend_user.auth.configuration import (
    OidcConfigurationStatus,
    inspect_oidc_configuration,
)
from frontend_user.auth.identity import (
...
from frontend_user.auth.providers import OAuthProvider, get_provider
from frontend_user.components.ui import APP_CSS, login_intro_html, profile_card_html
...
def _persist_profile(profile: IdentityProfile) -> PersistenceResult:
    """OIDC 인증 뒤 서명된 Backend API를 통해서만 로컬 계정을 연결한다."""

    return persist_identity(profile, secrets=st.secrets)
